...
module.exports = (env = {}, argv = {}, outputPath) => {
  ...
  const copyConfig = [
    ...
    { from: 'src/app/lmem/draft-preview/grabDraftRecommendations.js', to: path.join(buildPath, 'js')}
  ];
  if (argv.mode !== 'production') {
    copyConfig.push(
      { from: 'test/infrastructure', to: path.join(outputPath, env.build, 'test', 'infrastructure')},
      { from: 'test/integration', to: path.join(outputPath, env.build, 'test', 'integration') }
    );
  }
