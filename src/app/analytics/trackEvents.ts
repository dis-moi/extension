...
export function trackEvents(action: Action): void {
  const { heap } = window as CustomWindow;
  if (heap) {
    heap.track(action.type, mapEventFromAction(action));
  } else {
    console.info(
      `Heap analytics disabled: ignore tracking of "${action.type}"`
    );
  }
}
